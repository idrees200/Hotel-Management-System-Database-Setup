...
SELECT LEFT(guestname, charindex(' ', guestname) - 1) AS firstName
FROM guest

select guestname,guestaddress from guest
where guestaddress like'%Glasgow' and guestname like 'Tony%' or guestname like '%Farrel'

select r.roomno,r.hotelno ,booking.datefrom
from room as r, booking  
...
select top 1  roomno,hotelno,price from room
order by price desc

select concat ('THE HOTEL NO IS',hotelno,'THE CITY NO IS',city ,'NAME IS ',hotelname)
from hotel 

select  ('THE HOTEL NO IS'+hotelno+'THE CITY NO IS'+city +'NAME IS '+hotelname)
from hotel 

select room.hotelno,hotel.hotelname
from room,hotel
where type='double' or type='family'

select room.hotelno,hotel.hotelname
from room join hotel on room.hotelno=hotel.hotelno where room.type='family' or type='double'


select room.hotelno,hotel.hotelname
from room join hotel on room.hotelno=hotel.hotelno where (room.type='single' or type='double' and type !='family')

select room.hotelno,hotel.hotelname
from room,hotel
where type='double' or type='single' and type != 'family'

select distinct roomno
from booking
where ((guestno='10007' or guestno='10003') and(guestno !='10001'));
